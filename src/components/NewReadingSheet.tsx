                            ...
                            {step === "main" ? (
                                ...
                                >
                                    <div className="flex items-center justify-between mb-8 px-2 shrink-0">
                                        <button onClick={onClose} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center active:scale-90 transition-transform"><ChevronLeft className="w-6 h-6 text-white" /></button>
                                        <h3 className="text-xl font-black tracking-tight text-white uppercase italic">Új <span className="text-primary">Óraállás</span></h3>
                                        <div className="w-10" />
                                    </div>
                                    ...
                                    <div className="flex-1 overflow-y-auto space-y-6 px-2 scrollbar-hide pb-20">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            >
                                                ...
                                                <div className="w-14 h-14 rounded-2xl bg-indigo-500 flex items-center justify-center shadow-lg">
                                                    {isScanningAI ? <Loader2 className="animate-spin text-white w-6 h-6" /> : <Camera className="text-white w-6 h-6" />}
                                                </div>
                                        ...
                                        <div className="flex flex-col gap-2">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-4">Mérőóra</label>
                                            <div className="relative z-[60]">
                                                <button onClick={() => setIsSelectOpen(!isSelectOpen)} disabled={isLoadingMeters || meters.length === 0} className="w-full flex items-center justify-between bg-white/5 border border-white/5 rounded-2xl py-4 px-6 text-white font-bold transition-all active:scale-[0.98] disabled:opacity-50">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="flex flex-col gap-2">
                                                <label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-4">Időpont</label>
                                                <button onClick={() => { setTempDate(new Date(selectedDate)); setStep("date"); }} className="w-full flex items-center h-15 justify-between bg-white/5 border border-white/5 rounded-2xl py-4 px-4 text-white font-bold active:scale-[0.98]">
                                            ...
                                            <div className="flex flex-col gap-2">
                                                <label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-4">Állás ({activeMeter?.unit})</label>
                                                <div className="relative">
                                                    <input type="number" value={value} onChange={(e) => setValue(e.target.value)} placeholder="0" className="w-full bg-white/5 border border-white/5 rounded-2xl py-4 px-5 text-white font-black text-xl focus:outline-none focus:border-primary/50 transition-colors" />
                                                </div>
                                            </div>
                                        </div>
                                ...
                                >
                                    <div className="flex items-center justify-between mb-8 px-2 shrink-0">
                                        <button onClick={() => setStep("main")} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center active:scale-90 transition-transform"><ChevronLeft className="w-6 h-6 text-white" /></button>
                                        <h3 className="text-xl font-black text-white uppercase italic">Időpont</h3>
                                        <div className="w-10" />
                                    </div>

                                    <div className="flex bg-white/5 p-1 rounded-2xl mb-8 self-center">
                                        <button onClick={() => setPickerView("date")} className={`px-8 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${pickerView === "date" ? "bg-white text-black" : "text-white/40"}`}>Dátum</button>
                                        <button onClick={() => setPickerView("time")} className={`px-8 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${pickerView === "time" ? "bg-white text-black" : "text-white/40"}`}>Idő</button>
                                    </div>
                                        ...
                                        {pickerView === "date" ? (
                                            <div className="flex flex-col gap-6">
                                                <div className="flex items-center justify-between bg-white/5 p-2 rounded-2xl">
                                                    <button onClick={() => updateTempDate({ month: tempDate.getMonth() - 1 })} className="p-2 text-white/40"><ChevronLeft size={20} /></button>
                                                    ...
                                                    <button onClick={() => updateTempDate({ month: tempDate.getMonth() + 1 })} className="p-2 text-white/40"><ChevronRight size={20} /></button>
                                                </div>
                                                <div className="grid grid-cols-7 gap-1 justify-items-center">
                                                    {["H", "K", "Sze", "Cs", "P", "Szo", "V"].map((day) => (<div key={day} className="text-[10px] font-black text-white/20 text-center mb-2 uppercase">{day}</div>))}
                                                    ...
                                                    {Array.from({ length: daysInMonth(tempDate.getFullYear(), tempDate.getMonth()) }).map((_, i) => {
                                                        ...
                                                        return (
                                                            <button key={`day-${d}`} onClick={() => updateTempDate({ day: d })} className={`h-10 w-10 rounded-full flex items-center justify-center text-sm font-black transition-all ${isSelected ? "bg-primary text-white scale-110 shadow-lg" : "text-white/40 hover:bg-white/5"}`}>{d}</button>
                                                        );
                                                    })}
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex flex-col items-center justify-center pt-8 gap-8">
                                                <div className="flex items-center gap-6">
                                                    <input type="number" value={tempDate.getHours().toString().padStart(2, '0')} onChange={(e) => updateTempDate({ hour: parseInt(e.target.value) || 0 })} className="w-24 bg-white/5 border border-white/5 rounded-[2rem] py-8 text-center text-5xl font-black text-white focus:outline-none focus:border-primary/50" />
                                                    <span className="text-4xl font-black text-white/10">:</span>
                                                    <input type="number" value={tempDate.getMinutes().toString().padStart(2, '0')} onChange={(e) => updateTempDate({ minute: parseInt(e.target.value) || 0 })} className="w-24 bg-white/5 border border-white/5 rounded-[2rem] py-8 text-center text-5xl font-black text-white focus:outline-none focus:border-primary/50" />
                                                </div>
