...
export default function TimeRangeSheet({ isOpen, onClose, selectedFrequency, onSelect }: Props) {
    ...
    const options = [
        { id: "3m", label: "Utolsó 3 hónap", sub: "Negyedéves áttekintés" },
        { id: "6m", label: "Utolsó 6 hónap", sub: "Féléves statisztika" },
        { id: "1y", label: "Utolsó 1 év", sub: "Éves trendek" },
    ];
    ...
    const handleApplyCustom = () => {
        const startStr = `${startData.year}.${(startData.month + 1).toString().padStart(2, '0')}`;
        const endStr = `${endData.year}.${(endData.month + 1).toString().padStart(2, '0')}`;
        onSelect("custom", { start: startStr, end: endStr });
        setShowCustom(false);
        onClose();
    };
                            ...
                            {!showCustom ? (
                                <div className="space-y-3">
                                    {options.map((opt) => (
                                        <motion.button
                                            ...
                                            className={`w-full flex items-center justify-between p-6 rounded-[2.5rem] border transition-all ${selectedFrequency === opt.id ? "bg-primary/10 border-primary/20" : "bg-white/5 border-white/5"
                                                }`}
                                        >
                                    ...
                                    <motion.button
                                        ...
                                        className={`w-full flex items-center justify-between p-6 rounded-[2.5rem] border transition-all ${selectedFrequency === "custom" ? "bg-primary/10 border-primary/20" : "bg-white/5 border-white/5"
                                            }`}
                                    >
                                        <div className="flex items-center gap-5">
                                ...
                                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                                    ...
                                    <div className="grid grid-cols-3 gap-3 px-2">
                                        {months.map((m, i) => {
                                            const curYear = currentStep === "start" ? startData.year : endData.year;
                                            const curAbs = getAbs(curYear, i);
                                            const isStart = curAbs === startAbs;
                                            const isEnd = curAbs === endAbs;
                                            const isInRange = curAbs > rangeMin && curAbs < rangeMax;
                                    ...
                                    <button onClick={handleApplyCustom} className="w-full py-6 bg-white text-black rounded-[2rem] font-black uppercase tracking-[0.2em] text-sm shadow-2xl active:scale-95 transition-transform">Alkalmazás</button>
                                </motion.div>
                            )}
                        </div>
                    </motion.div>
