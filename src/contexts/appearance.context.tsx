...
export const WALLPAPERS = [
    // --- VILÁGÍTÓ (8 db - Fent sötét, lefelé világosodó stílus HEX színekkel) ---
    ...
    // --- ABSZTRAKT (8 db - Monokróm/Kéttónusú, elegáns formák) ---
    ...
    // --- SZÍNEK (8 db - ÚJ: Többszínű, komplex lávalámpa/aurora hatások) ---
    // s1: Kék, Lila, Pink Aurora
    ...
    // s2: Naplemente (Narancs, Piros, Sárga)
    ...
    // s3: Északi fény (Smaragd, Cián, Mélykék)
    ...
    // s6: Láva & Hamu (Sötétbordó, Izzó narancs)
    ...
    // s7: Kozmikus por (Mélylila, Rózsaszín, Kék)
    ...
    // s8: Trópusi éjszaka (Türkiz, Magenta, Sötétkék)
...
export function AppearanceProvider({ children, initialValues }: {
    ...
    initialValues: { theme: string; accent: string; animations: string; wallpaper: string }
}) {
    ...
    useEffect(() => {
        ...
        // Wallpaper kiválasztása és animáció ellenőrzése
    ...
    return (
        <AppearanceContext.Provider value={{
            theme, accent, animations, wallpaper,
            setTheme: (t) => { setThemeState(t); saveCookie("app_theme", t); },
            setAccent: (a) => { setAccentState(a); saveCookie("app_accent", a); },
            setAnimations: (en) => { setAnimationsState(en); saveCookie("app_animations", en.toString()); },
            setWallpaper
        }}>
